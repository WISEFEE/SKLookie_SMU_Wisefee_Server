...
public class ProductOptChoiceRepository {
    ...
    public List<ProductOptChoice> findByProductOption(ProductOption productOption) {
        return em.createQuery("select poc from ProductOptChoice poc join fetch poc.productOption where poc.productOption = :productOption and poc.deletedAt is null", ProductOptChoice.class )
                .setParameter("productOption", productOption)
                .getResultList();
    }

    /**
     * [ProductOptChoice 소프트 삭제로 업데이트]
     * ProductOptChoice 엔티티를 소프트 삭제로 업데이트한다.
     * @param productOptChoice 소프트 삭제할 ProductOptChoice 엔티티
     */
    public void softDelete(ProductOptChoice productOptChoice) {
        productOptChoice.setDeletedAt(LocalDateTime.now());
        em.persist(productOptChoice);
    }
}
